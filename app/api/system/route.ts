...
// Helper to get detailed CPU info
async function getDetailedCPUInfo(): Promise<{ vendor: string; model: string; hasNPU: boolean }> {
    const osType = getOS();
    let vendor = 'Unknown';
    ...
    try {
        if (osType === 'windows') {
            const { stdout: cpuOut } = await execAsync('wmic cpu get manufacturer, name /format:list');
            const lines = cpuOut.split('\n');
            const manufacturer = lines.find(l => l.includes('Manufacturer'))?.split('=')[1]?.trim() || '';
            const name = lines.find(l => l.includes('Name'))?.split('=')[1]?.trim() || '';

            vendor = manufacturer.includes('AuthenticAMD') ? 'AMD' : manufacturer.includes('GenuineIntel') ? 'Intel' : manufacturer;
            model = name || model;

            // Check for AMD Ryzen AI NPU (simplified check for demo purposes)
            // In a real app, we might check for the "AMD IPU" or "AMD NPU" device driver
            const { stdout: deviceOut } = await execAsync('powershell "Get-PnpDevice | Where-Object { $_.FriendlyName -match \'AMD IPU\' -or $_.FriendlyName -match \'AMD NPU\' } | Select-Object -First 1"');
            if (deviceOut.trim()) hasNPU = true;

        } else if (osType === 'linux') {
            const { stdout: cpuOut } = await execAsync('cat /proc/cpuinfo | grep -E "vendor_id|model name" | head -n 2');
            const lines = cpuOut.split('\n');
            const vendorLine = lines.find(l => l.includes('vendor_id'))?.split(':')[1]?.trim() || '';
            const modelLine = lines.find(l => l.includes('model name'))?.split(':')[1]?.trim() || '';

            vendor = vendorLine.includes('AuthenticAMD') ? 'AMD' : vendorLine.includes('GenuineIntel') ? 'Intel' : vendorLine;
            model = modelLine || model;

            // Check for AMD NPU in Linux (often appears in dmesg or specific sysfs paths)
            try {
                const { stdout: npuCheck } = await execAsync('dmesg | grep -i "amd ipu"');
                if (npuCheck.trim()) hasNPU = true;
            } catch { /* ignore */ }
        }
    } catch (e) {
    ...
    try {
        ...
        switch (action) {
            ...
            case 'startMonitor':
                ...
                const cpuInfo = await getDetailedCPUInfo();
                if (monitorResult.success && monitorResult.data) {
                    (monitorResult.data as any).cpuInfo = cpuInfo;
                }
